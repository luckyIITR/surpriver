...
class TAEngine:
    ...
    def calculate_slope(self, data):
        ...
        slope, r_value, p_value = round(regression_model.slope, 3), round(abs(regression_model.rvalue), 3), round(
            regression_model.pvalue, 4)
    ...
    def get_technical_indicators(self, price_data):
        ...
        for history in rsi_history:
            ...
            technical_indicators_dictionary["rsi-" + str(history)] = rsi[-self.HISTORY_TO_USE:] + [slope_rsi,
                                                                                                   r_value_rsi,
                                                                                                   p_value_rsi]
        ...
        for history in stochastic_history:
            stochs = ta.momentum.StochasticOscillator(price_data['High'], price_data['Low'], price_data['Close'],
                                                      window=history, smooth_window=int(history / 3),
                                                      fillna=True).stoch().values.tolist()
            slope_stoch, r_value_stoch, p_value_stoch = self.calculate_slope(stochs[-self.HISTORY_TO_USE:])
            technical_indicators_dictionary["stochs-" + str(history)] = stochs[-self.HISTORY_TO_USE:] + [slope_stoch,
                                                                                                         r_value_stoch,
                                                                                                         p_value_stoch]

        # Accumulation Distribution
        acc_dist = ta.volume.AccDistIndexIndicator(price_data['High'], price_data['Low'], price_data['Close'],
                                                   price_data['Volume'], fillna=True).acc_dist_index().values.tolist()
        ...
        for history in eom_history:
            eom = ta.volume.EaseOfMovementIndicator(price_data['High'], price_data['Low'], price_data['Volume'],
                                                    window=history, fillna=True).ease_of_movement().values.tolist()
        ...
        for history in cci_history:
            cci = ta.trend.CCIIndicator(price_data['High'], price_data['Low'], price_data['Close'], window=history,
                                        constant=0.015, fillna=True).cci().values.tolist()
            slope_cci, r_value_cci, p_value_cci = self.calculate_slope(cci[-self.HISTORY_TO_USE:])
            technical_indicators_dictionary["cci-" + str(history)] = cci[-self.HISTORY_TO_USE:] + [slope_cci,
                                                                                                   r_value_cci,
                                                                                                   p_value_cci]

        # Daily log return
        daily_return = ta.others.DailyReturnIndicator(price_data['Close'], fillna=True).daily_return().values.tolist()
        daily_log_return = ta.others.DailyLogReturnIndicator(price_data['Close'],
                                                             fillna=True).daily_log_return().values.tolist()
        ...
        technical_indicators_dictionary["volume_returns"] = volume_returns[-self.HISTORY_TO_USE:] + [slope_vol,
                                                                                                     r_value_vol,
                                                                                                     p_value_vol]
